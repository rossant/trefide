    ...
    double rho, double obj_tol, cs * DktDk, int verbose)
{
  ...
  if (verbose) fprintf(stdout,"Iteration\tObjective\n");
  ...
  for (it=0; it < max_iter; it++)
  {
    ...
    if (verbose) fprintf(stdout,"%i\t%0.3e\n",it+1,obj[it]);
    ...
    cs * DktDk, func_RtoR b, func_RtoR b1, func_RtoR b2, int verbose)
{
  ...
  if (verbose) fprintf(stdout, "\nlambda=%0.3e\n",lam);
  if (verbose) fprintf(stdout, "Iteration\tObjective\tADMM iters\n");
  ...
  for (it=0; it < max_iter_newton; it++)
  {
    ...
    if (verbose) fprintf(stdout, "\t%i\t%0.3e\t%i\t%i\n",it+1,obj[it],iter_admm,*iter_ls);
